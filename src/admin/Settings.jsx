...
const Settings = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(`https://ecom-api-2xbg.onrender.com/api/user/${user._id}`, users, {
        headers: {
          "Authorization": `Bearer ${localStorage.getItem('token')}`
        }
      })
  ...
  return (
    <> {user ? (
      <div className="max-w-2xl mx-auto p-6 bg-white shadow rounded-lg mt-10">
        <h1 className="text-2xl font-bold mb-6 border-b pb-3">Profile Settings</h1>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div>
              ...
              <input
                ...
                className="mt-1 w-full text-gray-900 border rounded p-2 bg-gray-50"
                required />
            </div>
            ...
            <button type="submit" className="mt-6 px-5 py-2 bg-blue-500 text-white rounded hover:bg-blue-600">
              Save Changes
            </button>
          </form>
        </div>
